...
# 读取vnpy相关配置中的不同品种交易时间配置
FUTURE_TRADE_TIME_JSON_PATH: str = 'trading_time.json'
TIME_FORMAT = '%H:%M:%S'
...
# 读取不同品种交易时间文件, 目前只有期货交易时间(股指+国债+商品)
trading_time = {}
if Path(FUTURE_TRADE_TIME_JSON_PATH).exists():
    with open(FUTURE_TRADE_TIME_JSON_PATH, 'r', encoding='utf8') as f:
        future_trading_time = json.load(f)
    trading_time.update(future_trading_time)
...
def extract_variety_from_symbol(symbol: str) -> str:
    """ 从symbol中得到品种, symbol不包含Exchange """
    match_str = re.findall(r'([a-zA-Z]*)\d+', symbol)
    if len(match_str) == 0:
        return ""
    return match_str[0]

def symbol_close_time(symbol: str) -> Type[time]:
    """ 获取symbol的收盘时间, symbol为不包含Exchange的代码 """
    variety = extract_variety_from_symbol(symbol)
    return variety_close_time(variety)

def variety_close_time(variety: str) -> Type[time]:
    """ 获取品种的收盘时间, 默认为下午15点15分 """
    symbol_trade_time_intervals = trading_time.get(variety, None)
    if symbol_trade_time_intervals is None:
        return time(hour=15, minute=15)
    return datetime.strptime(symbol_trade_time_intervals[-1][-1], TIME_FORMAT).time()
...
def _get_trader_dir(temp_name: str, cwd: Optional[str]=None) -> Tuple[Path, Path]:
    """
...
work_dir = read_vnpy_config('WORK_DIR', 'DIR')
work_folder = read_vnpy_config('WORK_DIR', 'FOLDER')

TRADER_DIR, TEMP_DIR = _get_trader_dir(work_folder, work_dir)
...
class BarGenerator:
    ...
    def should_call_on_bar(self, tick: TickData) -> bool:
        """ 根据传入的interval值分别判断是否应该调用策略的on_bar函数 """
        tick_time = tick.datetime
        bar_time = self.bar.datetime

        # 如果策略接收分钟K线, 则判断最新tick和已合成的K线的分钟或者小时数是否相同
        if self.interval == Interval.MINUTE:
            return bar_time.minute != tick_time.minute or \
                bar_time.hour != tick_time.hour
        # 如果策略接收小时K线, 则判断最新tick和已合成的K线的小时数是否相同
        if self.interval == Interval.HOUR:
            return bar_time.hour != tick_time.hour
        # * 如果策略接收日K线, 则判断已合成的K线的时间和最新tick时间是否跨过了品种的收盘时间
        if self.interval == Interval.DAILY:
            close_time = symbol_close_time(tick.symbol)
            return bar_time.time() < close_time and \
                tick_time.time() >= close_time
    ...
    def update_tick(self, tick: TickData) -> None:
        ...
        if not self.bar:
            new_bar = True
        elif self.should_call_on_bar(tick):
        ...
        if new_bar:
            ...
            if self.last_tick is not None and tick.high_price > self.last_tick.high_price:
                self.bar.high_price = max(self.bar.high_price, tick.high_price)

            self.bar.low_price = min(self.bar.low_price, tick.last_price)
            if self.last_tick is not None and tick.low_price < self.last_tick.low_price:
